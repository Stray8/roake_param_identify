...
void torqueControl(xMateErProRobot &robot) 
{
    ...
    position_file.open("./ly_data/testset/position_ly.txt");
    position_error_file.open("./ly_data/testset/position_error_ly.txt");
    velocity_file.open("./ly_data/testset/velocity_file.txt");
    acceleration_file.open("./ly_data/testset/acceleration_file.txt");
    inertia_file.open("./ly_data/testset/inertia_file.txt");
    coriolis_file.open("./ly_data/testset/coriolis_file.txt");
    gravity_file.open("./ly_data/testset/gravity_file.txt");
    torque_file.open("./ly_data/testset/torque_file.txt");
    ...
    std::function<Torque(void)> callback = [&]
    {
      ...
      //定义参考轨迹
      // '''参考位置'''
      double delta_angle0 = M_PI / 50.0 * (1 - std::cos(M_PI / 2 * 1 * time));
      double delta_angle1 = M_PI / 40.0 * (1 - std::cos(M_PI / 3 * 4 * time));
      double delta_angle2 = M_PI / 50.0 * (1 - std::cos(M_PI / 7 * 1 * time));
      double delta_angle3 = M_PI / 70.0 * (1 - std::cos(M_PI / 5 * time));
      double delta_angle4 = M_PI / 60.0 * (1 - std::cos(M_PI / 6* time));
      double delta_angle5 = M_PI / 60.0 * (1 - std::cos(M_PI / 2 * 4 * time));
      double delta_angle6 = M_PI / 70.0 * (1 - std::cos(M_PI / 2 * 3 * time));
      // '''参考速度 '''
      double dot_delta_angle0 = M_PI / 50.0 * M_PI / 2 * 1 * std::sin(M_PI / 4 * 1 * time);
      double dot_delta_angle1 = M_PI / 40.0 * M_PI / 3 * 4 * std::sin(M_PI / 4 * 3 * time);
      double dot_delta_angle2 = M_PI / 50.0 * M_PI / 7 * 1 * std::sin(M_PI / 2 * 1 * time);
      double dot_delta_angle3 = M_PI / 70.0 * M_PI / 5 * std::sin(M_PI * time);
      double dot_delta_angle4 = M_PI / 60.0 * M_PI / 6 * std::sin(M_PI * time);
