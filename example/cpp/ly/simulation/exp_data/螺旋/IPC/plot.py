...
D_trajectory = np.loadtxt('D_trajectory.txt')[0:550, :]
dpx = D_trajectory[:, 0]
dpy = D_trajectory[:, 2]
...
thres_y = np.arange(-0.2, 0.05, 0.01)
thres_x1 = np.ones(np.shape(thres_y)[0]) * 0.2
thres_x2 = np.ones(np.shape(thres_y)[0]) * 0.35
thres_x = np.arange(0.15, 0.4, 0.01)
thres_y1 = np.ones(np.shape(thres_x)[0]) * 0
thres_y2 = np.ones(np.shape(thres_x)[0]) * (-0.15)
plt1.plot(thres_x1, thres_y, ls=':', c='grey')
plt1.plot(thres_x2, thres_y, ls=':', c='grey')
plt1.plot(thres_x, thres_y1, ls=':', c='grey')
plt1.plot(thres_x, thres_y2, ls=':', c='grey')
...
thres1 = np.ones(np.shape(rx)[0]) * 0.35
thres2 = np.ones(np.shape(rx)[0]) * 0.2
plt2.plot(np.arange(np.shape(thres1)[0]), thres1, c='grey', ls=':')
plt2.plot(np.arange(np.shape(thres2)[0]), thres2, c='grey', ls=':')
...
thres1 = np.ones(np.shape(rx)[0]) * 0
thres2 = np.ones(np.shape(rx)[0]) * (-0.15)
plt3.plot(np.arange(np.shape(thres1)[0]), thres1, c='grey', ls=':')
plt3.plot(np.arange(np.shape(thres2)[0]), thres2, c='grey', ls=':')
...
plt4.set_ylabel('Force (N)', fontname='Times New Roman', fontsize=12)
plt4.set_title('(d): Force trajectories', y=-0.55, fontname='Times New Roman', fontsize=12)
...
plt5.set_ylabel('Stiffness (N/m)', fontname='Times New Roman', fontsize=12)
plt5.set_title('(e): Stiffness trajectories', y=-0.55, fontname='Times New Roman', fontsize=12)
plt5.legend(loc='upper left', prop=font1, frameon=False, handlelength=1, ncol=1, columnspacing=1)
plt5.tick_params(labelsize=12)

plt.savefig("C:/Users/dqjzh/Desktop/impedance_robot/ICRA2022/simulation/figs/SpiralIPC.jpg", dpi=150)
plt.savefig("C:/Users/dqjzh/Desktop/impedance_robot/ICRA2022/simulation/figs/SpiralIPC.pdf", dpi=1200)
plt.show()
